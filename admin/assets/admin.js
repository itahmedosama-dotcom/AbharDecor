...
async function verifyLogin(username,password){if(!cfg.API_URL)throw new Error('رابط Google Apps Script غير مضبوط');const r=await fetch(cfg.API_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'login',username,password})});const j=await r.json();if(!j.ok)throw new Error(j.error||'بيانات الدخول غير صحيحة');return true}
